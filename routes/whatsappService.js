...
async function obterDestino(client, telefone) {
    const numeroFormatado = formatarNumero(telefone);
    if (!numeroFormatado) return { error: "no_phone" };

    try {
        const numberId = await client.getNumberId(numeroFormatado);
        if (!numberId) return { error: "not_exists" };
        return {
            numeroFormatado,
            chatId: numberId._serialized
        };
    } catch (err) {
        console.log("⚠️ getNumberId falhou:", err.message);
        return { error: "number_error", detail: err.message };
    }
}
...
async function enviarMensagem(empresaId, telefone, mensagem) {
    try {
        ...
        const destino = await obterDestino(client, telefone);
        if (destino.error) return { ok: false, ...destino };

        await client.sendMessage(destino.chatId, String(mensagem).trim());
        console.log("✅ Whats enviado:", destino.numeroFormatado);
        return { ok: true, numero: destino.numeroFormatado };
    } catch (err) {
...
async function enviarDocumento(
    ...
    try {
        const client = getCliente(empresaId);
        if (!client || !client.info) return { ok: false, error: "offline" };
        if (!Buffer.isBuffer(arquivoBuffer) || !arquivoBuffer.length) {
        ...
        const media = new MessageMedia(
            "application/pdf",
            arquivoBuffer.toString("base64"),
            nomeArquivo
        );

        await client.sendMessage(destino.chatId, media, {
            caption: String(legenda || "").trim(),
            sendMediaAsDocument: true
        });

        console.log("✅ PDF enviado por WhatsApp:", destino.numeroFormatado);
        return {
            ok: true,
            numero: destino.numeroFormatado,
            arquivo: nomeArquivo
        };
    } catch (err) {
        console.log("❌ Erro enviando documento WhatsApp:", err.message);
        return { ok: false, error: "send_failed", detail: err.message };
    }
}
