...
const criarInadimplenciaService = require('./asaas.inadimplencia.service');

module.exports = function criarAsaasWebhookService(pool) {
    const inadimplencia = criarInadimplenciaService(pool);
    ...
    async function atualizarCobrancaLocal(local, payment, {
        ...
        if (statusInterno === 'PAGO') {
            await inadimplencia.reativarEmpresaSeRegular(local.empresa_id, {
                cobrancaId: local.id,
                usuarioId,
                origem
            });
        } else if (statusInterno === 'VENCIDO') {
            await inadimplencia.programarSuspensaoPorCobranca(local.id, {
                origem,
                usuarioId
            });
        }
