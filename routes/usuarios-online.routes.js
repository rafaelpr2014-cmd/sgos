const express = require("express");

module.exports = (pool, verificarAutenticacao) => {
    const router = express.Router();
    const OFFLINE_MINUTOS = 5;

    function somenteEmpresa1(req, res, next) {
        if (Number(req.usuario?.empresa_id) !== 1) {
            return res.status(403).json({
                erro: "Página disponível somente para usuários da empresa 1."
            });
        }
        next();
    }

    router.use(verificarAutenticacao, somenteEmpresa1);

    router.get("/acesso", (req, res) => {
        res.json({
            permitido: true,
            usuario: {
                id: req.usuario.id,
                nome: req.usuario.usuario,
                cargo: req.usuario.cargo,
                empresa_id: req.usuario.empresa_id
            }
        });
    });

    router.get("/usuarios", async (req, res) => {
        try {
            await pool.query(
                `UPDATE log_acessos
                 SET status = 'offline'
                 WHERE logout IS NULL
                   AND status = 'ativo'
                   AND ultimo_ping IS NOT NULL
                   AND ultimo_ping <= DATE_SUB(NOW(), INTERVAL ? MINUTE)`,
                [OFFLINE_MINUTOS]
            );

            const [usuarios] = await pool.query(`
                SELECT
                    u.id,
                    u.usuario AS nome,
                    u.usuario AS login,
                    NULL AS email,
                    u.cargo AS tipo,
                    u.empresa_id,
                    COALESCE(e.nome_provedor, CONCAT('Empresa ', u.empresa_id)) AS empresa_nome,
                    CASE
                        WHEN la.id IS NOT NULL
                         AND la.logout IS NULL
                         AND la.status = 'ativo'
                         AND la.ultimo_ping > DATE_SUB(NOW(), INTERVAL ${OFFLINE_MINUTOS} MINUTE)
                        THEN 'online'
                        ELSE 'offline'
                    END AS status,
                    la.login AS conectado_em,
                    la.ultimo_ping AS ultima_atividade,
                    la.logout AS desconectado_em,
                    la.ip_origem AS ip,
                    la.porta_origem,
                    NULL AS user_agent
                FROM usuarios u
                LEFT JOIN empresa e ON e.id = u.empresa_id
                LEFT JOIN log_acessos la ON la.id = (
                    SELECT MAX(l2.id)
                    FROM log_acessos l2
                    WHERE l2.usuario_id = u.id
                )
                WHERE COALESCE(u.ativo, 1) = 1
                ORDER BY
                    CASE
                        WHEN la.id IS NOT NULL
                         AND la.logout IS NULL
                         AND la.status = 'ativo'
                         AND la.ultimo_ping > DATE_SUB(NOW(), INTERVAL ${OFFLINE_MINUTOS} MINUTE)
                        THEN 0 ELSE 1
                    END,
                    empresa_nome,
                    u.usuario
            `);

            const online = usuarios.filter(usuario => usuario.status === "online").length;
            const empresas = new Set(usuarios.map(usuario => Number(usuario.empresa_id))).size;

            res.json({
                usuarios,
                resumo: {
                    total_usuarios: usuarios.length,
                    online,
                    offline: usuarios.length - online,
                    empresas
                }
            });
        } catch (err) {
            console.error("ERRO AO LISTAR USUÁRIOS ONLINE:", err);
            res.status(500).json({ erro: err.message });
        }
    });

    router.get("/usuarios/:usuarioId/logs", async (req, res) => {
        const usuarioId = Number(req.params.usuarioId);
        const limite = Math.min(Math.max(Number(req.query.limite) || 100, 1), 100);

        if (!usuarioId) {
            return res.status(400).json({ erro: "Usuário inválido." });
        }

        try {
            const [logsAcesso] = await pool.query(`
                SELECT
                    id,
                    usuario_id,
                    empresa_id,
                    CASE
                        WHEN logout IS NULL THEN 'conexao'
                        ELSE 'desconexao'
                    END AS tipo,
                    ip_origem AS ip,
                    porta_origem,
                    motivo_logout AS motivo,
                    login AS ocorrido_em,
                    login,
                    logout,
                    ultimo_ping,
                    status
                FROM log_acessos
                WHERE usuario_id = ?
                ORDER BY id DESC
                LIMIT ?
            `, [usuarioId, limite]);

            const logs = [];

            for (const item of logsAcesso) {
                logs.push({
                    id: `${item.id}-login`,
                    usuario_id: item.usuario_id,
                    empresa_id: item.empresa_id,
                    tipo: "conexao",
                    ip: item.ip,
                    porta_origem: item.porta_origem,
                    user_agent: null,
                    motivo: "Login realizado",
                    ocorrido_em: item.login
                });

                if (item.logout) {
                    logs.push({
                        id: `${item.id}-logout`,
                        usuario_id: item.usuario_id,
                        empresa_id: item.empresa_id,
                        tipo: "desconexao",
                        ip: item.ip,
                        porta_origem: item.porta_origem,
                        user_agent: null,
                        motivo: item.motivo || "Logout registrado",
                        ocorrido_em: item.logout
                    });
                }
            }

            logs.sort((a, b) => new Date(b.ocorrido_em) - new Date(a.ocorrido_em));

            res.json({ logs: logs.slice(0, limite), limite });
        } catch (err) {
            console.error("ERRO AO CARREGAR HISTÓRICO DO USUÁRIO:", err);
            res.status(500).json({ erro: err.message });
        }
    });

    return router;
};
