...
router.get('/movimentacoes', somenteAdmin, async(req,res)=>{
  try{
    ...
    const [rows]=await pool.query(`SELECT m.*,p.nome produto_nome,p.codigo produto_codigo,p.escritorio_id,e.nome escritorio_nome,COALESCE(NULLIF(GROUP_CONCAT(DISTINCT mt.tecnico_nome ORDER BY mt.tecnico_nome SEPARATOR ', '),''),
      (SELECT GROUP_CONCAT(DISTINCT t.nome ORDER BY t.nome SEPARATOR ', ')
         FROM ordens_servico os2
         JOIN tecnicos t ON t.empresa_id=os2.empresa_id
          AND FIND_IN_SET(CAST(t.id AS CHAR),REPLACE(REPLACE(REPLACE(COALESCE(os2.tecnico,''),'[',''),']',''),' ',''))>0
        WHERE os2.id=m.os_id AND os2.empresa_id=m.empresa_id)) tecnicos_nomes FROM estoque_movimentacoes m JOIN estoque_produtos p ON p.id=m.produto_id LEFT JOIN escritorios e ON e.id=p.escritorio_id AND e.empresa_id=p.empresa_id LEFT JOIN estoque_movimentacao_tecnicos mt ON mt.movimentacao_id=m.id WHERE ${f.join(' AND ')} GROUP BY m.id ORDER BY m.criado_em DESC LIMIT 2000`,p);res.json(rows);
