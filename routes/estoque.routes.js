...
router.get('/resumo', somenteAdmin, async (req, res) => {
  try {
    ...
    const [alertas] = await pool.query(
      `SELECT id,nome,quantidade,estoque_minimo,unidade_medida,localidade FROM estoque_produtos
       WHERE empresa_id=? AND ativo=1 AND quantidade<=estoque_minimo ORDER BY quantidade,nome`, [empresaId]);
...
router.get('/produtos', somenteAdmin, async (req, res) => {
  try {
    const empresaId = getEmpresaId(req), termo = String(req.query.busca || '').trim(), localidade = String(req.query.localidade || '').trim();
    const filtros = ['p.empresa_id=?','p.ativo=1'], params=[empresaId];
    if (termo) { filtros.push('(p.nome LIKE ? OR p.codigo LIKE ? OR p.categoria LIKE ? OR p.localidade LIKE ?)'); const like=`%${termo}%`; params.push(like,like,like,like); }
    if (localidade) { filtros.push('p.localidade=?'); params.push(localidade); }
    if (String(req.query.estoque_baixo || '') === '1') filtros.push('p.quantidade<=p.estoque_minimo');
    const [produtos] = await pool.query(
      `SELECT p.*,u.usuario cadastrado_por_nome,
       CASE WHEN p.quantidade=0 THEN 'sem_estoque' WHEN p.quantidade<=p.estoque_minimo THEN 'baixo' ELSE 'normal' END nivel_estoque
       FROM estoque_produtos p LEFT JOIN usuarios u ON u.id=p.cadastrado_por
...
router.post('/produtos', somenteAdmin, async (req, res) => {
  ...
  try {
    ...
    const { nome,codigo,categoria,unidade_medida='unidade',localidade,observacao='' }=req.body||{};
    ...
    const [result]=await conn.query(
      `INSERT INTO estoque_produtos (empresa_id,nome,codigo,categoria,unidade_medida,localidade,quantidade,estoque_minimo,observacao,cadastrado_por,ativo,criado_em,atualizado_em)
       VALUES (?,?,?,?,?,?,?,?,?,?,1,NOW(),NOW())`,
      [empresaId,String(nome).trim(),String(codigo||'').trim()||null,String(categoria||'').trim()||null,String(unidade_medida).trim(),String(localidade).trim(),quantidade,minimo,String(observacao||'').trim()||null,usuarioId]);
    const produto={id:result.insertId,nome:String(nome).trim(),quantidade,estoque_minimo:minimo};
    await registrarLog(conn,req,'produto_adicionado',produto,{depois:{nome,codigo,categoria,unidade_medida,localidade,quantidade,estoque_minimo:minimo,observacao},descricao:'Produto adicionado ao estoque.'});
    if (quantidade>0) await conn.query(
      `INSERT INTO estoque_movimentacoes (empresa_id,produto_id,tipo,quantidade,quantidade_anterior,quantidade_atual,motivo,usuario_id,usuario_nome,criado_em)
       VALUES (?,?,'entrada',?,0,?,'Estoque inicial',?,?,NOW())`, [empresaId,result.insertId,quantidade,quantidade,usuarioId,getUsuarioNome(req)]);
...
router.put('/produtos/:id', somenteAdmin, async (req,res)=>{
  const conn=await pool.getConnection();
  try{
    const empresaId=getEmpresaId(req), id=Number(req.params.id), {nome,codigo,categoria,unidade_medida,localidade,observacao}=req.body||{};
    if(!String(nome||'').trim()||!String(localidade||'').trim()) return res.status(400).json({erro:'Informe nome e localidade.'});
    ...
    const antes=rows[0];
    const depois={nome:String(nome).trim(),codigo:String(codigo||'').trim()||null,categoria:String(categoria||'').trim()||null,unidade_medida:String(unidade_medida||'unidade').trim(),localidade:String(localidade).trim(),estoque_minimo:minimo,observacao:String(observacao||'').trim()||null};
    await conn.query(`UPDATE estoque_produtos SET nome=?,codigo=?,categoria=?,unidade_medida=?,localidade=?,estoque_minimo=?,observacao=?,atualizado_em=NOW() WHERE id=? AND empresa_id=?`,[depois.nome,depois.codigo,depois.categoria,depois.unidade_medida,depois.localidade,depois.estoque_minimo,depois.observacao,id,empresaId]);
...
router.post('/movimentacoes', somenteAdmin, async(req,res)=>{
  ...
  try{
    ...
    const [mov]=await conn.query(`INSERT INTO estoque_movimentacoes (empresa_id,produto_id,tipo,quantidade,quantidade_anterior,quantidade_atual,motivo,observacao,usuario_id,usuario_nome,criado_em) VALUES (?,?,?,?,?,?,?,?,?,?,NOW())`,[empresaId,produtoId,movimento,qtd,anterior,atual,String(req.body?.motivo||'').trim()||'Movimentação de estoque',String(req.body?.observacao||'').trim()||null,usuarioId,getUsuarioNome(req)]);
...
router.get('/movimentacoes', somenteAdmin, async(req,res)=>{
  try{
    const empresaId=getEmpresaId(req),{tipo,produto_id,data_inicio,data_fim,usuario,tecnico_id}=req.query,f=['m.empresa_id=?'],p=[empresaId];
    if(tipo&&['entrada','saida','ajuste'].includes(tipo)){f.push('m.tipo=?');p.push(tipo);} if(produto_id){f.push('m.produto_id=?');p.push(Number(produto_id));}
    ...
    const [rows]=await pool.query(`SELECT m.*,p.nome produto_nome,p.codigo produto_codigo,p.localidade,GROUP_CONCAT(mt.tecnico_nome ORDER BY mt.tecnico_nome SEPARATOR ', ') tecnicos_nomes FROM estoque_movimentacoes m JOIN estoque_produtos p ON p.id=m.produto_id LEFT JOIN estoque_movimentacao_tecnicos mt ON mt.movimentacao_id=m.id WHERE ${f.join(' AND ')} GROUP BY m.id ORDER BY m.criado_em DESC LIMIT 2000`,p);res.json(rows);
