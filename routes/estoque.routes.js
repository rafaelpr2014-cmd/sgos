...
router.get('/movimentacoes', somenteAdmin, async(req,res)=>{
  try{
    ...
    const [rows]=await pool.query(`SELECT m.*,p.nome produto_nome,p.codigo produto_codigo,p.escritorio_id,p.valor_unitario,e.nome escritorio_nome,GROUP_CONCAT(mt.tecnico_nome ORDER BY mt.tecnico_nome SEPARATOR ', ') tecnicos_nomes FROM estoque_movimentacoes m JOIN estoque_produtos p ON p.id=m.produto_id LEFT JOIN escritorios e ON e.id=p.escritorio_id AND e.empresa_id=p.empresa_id LEFT JOIN estoque_movimentacao_tecnicos mt ON mt.movimentacao_id=m.id WHERE ${f.join(' AND ')} GROUP BY m.id ORDER BY m.criado_em DESC LIMIT 2000`,p);res.json(rows);
