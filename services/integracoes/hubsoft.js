const axios = require("axios");
const { normalizarClientePadrao, primeiroValor } = require("./interface");

function limparBaseUrl(baseUrl){
    const base = String(baseUrl || "").trim().replace(/\/+$/, "");
    if(!base) throw new Error("URL Base da HubSoft não informada.");
    return base;
}

function montarHeaders(config){
    const token = String(config.token || "").trim();
    if(!token) throw new Error("Token da HubSoft não informado.");

    return {
        "Authorization": /^Bearer\s+/i.test(token) ? token : `Bearer ${token}`,
        "Accept": "application/json",
        "Content-Type": "application/json"
    };
}

async function request(config, method, path, options = {}){
    const baseUrl = limparBaseUrl(config.base_url);
    const url = `${baseUrl}/${String(path || "").replace(/^\/+/, "")}`;

    const resposta = await axios({
        method,
        url,
        headers: montarHeaders(config),
        params: options.params || undefined,
        data: options.data || undefined,
        timeout: Number(process.env.HUBSOFT_TIMEOUT_MS || 15000),
        validateStatus: status => status >= 200 && status < 300
    });

    return resposta.data;
}

function extrairLista(data){
    if(Array.isArray(data)) return data;
    if(Array.isArray(data?.clientes)) return data.clientes;
    if(Array.isArray(data?.data)) return data.data;
    if(Array.isArray(data?.results)) return data.results;
    if(Array.isArray(data?.registros)) return data.registros;
    if(data?.cliente) return [data.cliente];
    if(data && typeof data === "object" && (data.id || data.codigo_cliente || data.nome_razaosocial)) return [data];
    return [];
}

function normalizarHubSoft(cliente = {}){
    const contrato = primeiroValor(cliente.contrato, cliente.contrato_servico, cliente.servico) || {};
    const endereco = primeiroValor(cliente.endereco, cliente.endereco_principal, cliente.endereco_instalacao) || {};
    const login = primeiroValor(cliente.login, cliente.login_pppoe, cliente.usuario_pppoe) || {};
    const plano = primeiroValor(cliente.plano, cliente.plano_servico, cliente.servico) || {};

    return normalizarClientePadrao({
        origem_erp: "hubsoft",
        id: primeiroValor(cliente.id, cliente.codigo_cliente, cliente.cliente_id),
        cliente_id: primeiroValor(cliente.id, cliente.codigo_cliente, cliente.cliente_id),
        contrato_id: primeiroValor(cliente.contrato_id, contrato.id, contrato.codigo_contrato, contrato.id_contrato),
        nome: primeiroValor(cliente.nome_razaosocial, cliente.nome, cliente.razao_social, cliente.fantasia),
        fantasia: primeiroValor(cliente.nome_fantasia, cliente.fantasia),
        cpf_cnpj: primeiroValor(cliente.cpf_cnpj, cliente.cnpj_cpf, cliente.documento, cliente.cpf, cliente.cnpj),
        telefone: primeiroValor(cliente.telefone, cliente.celular, cliente.whatsapp, cliente.fone),
        login: primeiroValor(cliente.usuario, cliente.login_pppoe, login.login, login.usuario),
        senha: primeiroValor(cliente.senha, cliente.senha_pppoe, login.senha, login.password),
        senha_pppoe: primeiroValor(cliente.senha_pppoe, cliente.senha, login.senha, login.password),
        plano_nome: primeiroValor(cliente.plano_nome, plano.nome, plano.descricao),
        plano_nome_erp: primeiroValor(cliente.plano_nome, plano.nome, plano.descricao),
        plano_id_erp: primeiroValor(cliente.plano_id, plano.id, plano.codigo),
        endereco: typeof endereco === "string" ? endereco : primeiroValor(endereco.completo, endereco.endereco_completo),
        rua: primeiroValor(cliente.rua, endereco.rua, endereco.logradouro, endereco.endereco),
        numero: primeiroValor(cliente.numero, endereco.numero),
        bairro: primeiroValor(cliente.bairro, endereco.bairro),
        cidade: primeiroValor(cliente.cidade, endereco.cidade, endereco.nome_cidade),
        referencia: primeiroValor(cliente.referencia, endereco.referencia, cliente.complemento, endereco.complemento),
        latitude: primeiroValor(cliente.latitude, endereco.latitude),
        longitude: primeiroValor(cliente.longitude, endereco.longitude),
        status: primeiroValor(cliente.status, cliente.ativo, contrato.status),
        bruto: { cliente, contrato, login, plano, endereco }
    }, "hubsoft");
}

async function testarConexao(config){
    const tentativas = [
        { method: "get", path: "api/v1/clientes", params: { per_page: 1 } },
        { method: "get", path: "api/v1/cliente", params: { per_page: 1 } },
        { method: "get", path: "clientes", params: { per_page: 1 } },
        { method: "get", path: "cliente", params: { per_page: 1 } }
    ];

    let ultimoErro = null;
    for(const t of tentativas){
        try{
            const retorno = await request(config, t.method, t.path, { params: t.params });
            return { ok: true, mensagem: "HubSoft conectado com sucesso.", endpoint: t.path, retorno };
        }catch(err){
            ultimoErro = err;
        }
    }

    throw ultimoErro || new Error("Não foi possível conectar na HubSoft.");
}

async function buscarClientes(config, termo){
    const busca = String(termo || "").trim();
    if(!busca) return { origem_erp: "hubsoft", total: 0, clientes: [] };

    const tentativas = [
        { method: "get", path: "api/v1/clientes", params: { busca, search: busca, termo: busca } },
        { method: "get", path: "api/v1/cliente", params: { busca, search: busca, termo: busca } },
        { method: "get", path: `api/v1/clientes/${encodeURIComponent(busca)}` },
        { method: "get", path: `api/v1/cliente/${encodeURIComponent(busca)}` },
        { method: "get", path: "clientes", params: { busca, search: busca, termo: busca } },
        { method: "get", path: "cliente", params: { busca, search: busca, termo: busca } }
    ];

    let ultimoErro = null;
    for(const t of tentativas){
        try{
            const retorno = await request(config, t.method, t.path, { params: t.params });
            const clientes = extrairLista(retorno).map(normalizarHubSoft);
            if(clientes.length){
                return { origem_erp: "hubsoft", total: clientes.length, clientes, bruto: retorno, endpoint: t.path };
            }
        }catch(err){
            ultimoErro = err;
        }
    }

    if(ultimoErro && ultimoErro.response?.status && ![404, 422].includes(ultimoErro.response.status)){
        throw ultimoErro;
    }

    return { origem_erp: "hubsoft", total: 0, clientes: [] };
}

async function buscarCliente(config, termo){
    const resposta = await buscarClientes(config, termo);
    return resposta.clientes?.[0] || null;
}

async function buscarClienteCompleto(config, termo){
    return buscarCliente(config, termo);
}

async function buscarContrato(config, clienteOuTermo){
    if(typeof clienteOuTermo === "object") return clienteOuTermo.bruto?.contrato || null;
    const cliente = await buscarClienteCompleto(config, clienteOuTermo);
    return cliente?.bruto?.contrato || null;
}

async function buscarLogin(config, clienteOuTermo){
    if(typeof clienteOuTermo === "object") return clienteOuTermo.bruto?.login || clienteOuTermo.login || null;
    const cliente = await buscarClienteCompleto(config, clienteOuTermo);
    return cliente?.bruto?.login || cliente?.login || null;
}

async function buscarPlano(config, clienteOuTermo){
    if(typeof clienteOuTermo === "object"){
        return clienteOuTermo.bruto?.plano || { id: clienteOuTermo.plano_id_erp || "", nome: clienteOuTermo.plano_nome || "" };
    }
    const cliente = await buscarClienteCompleto(config, clienteOuTermo);
    return cliente ? (cliente.bruto?.plano || { id: cliente.plano_id_erp || "", nome: cliente.plano_nome || "" }) : null;
}

module.exports = {
    endpointTeste: "/api/v1/clientes",
    endpointClientes: "/api/v1/clientes",
    request,
    testarConexao,
    buscarClientes,
    buscarCliente,
    buscarClienteCompleto,
    buscarClientePorId: buscarClienteCompleto,
    buscarContrato,
    buscarLogin,
    buscarPlano
};
