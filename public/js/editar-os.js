...
async function carregarOS() {
    try {
        const os = await fetchAuth("/api/ordens_servico/" + osId);

        if (!os) return alert("OS não encontrada");

        console.log("OS carregada:", os);
        console.log("Número carregado:", os.n);
...
document.getElementById("formEditarOS").addEventListener("submit", async (e) => {
    ...
    const payload = {
        ...
        n: document.getElementById("n") ? document.getElementById("n").value : "",
