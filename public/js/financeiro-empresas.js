...
<td><strong>${esc(nomeEmpresa(e))}</strong><div class="muted">${esc(e.subdominio||'sem subdomínio')}</div></td>
...
async function carregarSolicitacoesPromessa(){const b=$('solicitacoesBody');if(!b)return;try{const l=await api('/api/asaas/solicitacoes-promessa?status=PENDENTE');b.innerHTML=l.length?l.map(s=>`<tr><td>${esc(s.nome_fantasia||s.nome_provedor||s.razao_social||'Empresa sem nome')}</td><td>${esc(s.competencia||'-')}</td><td>${moeda(s.valor)}</td><td>${dataBR(s.data_solicitada)}</td><td>${esc(s.status)}</td><td>${esc(s.observacao||'-')}</td><td><button class="btn btn-success" data-aprovar="${s.id}">Aprovar</button> <button class="btn btn-danger" data-recusar="${s.id}">Recusar</button></td></tr>`).join(''):'<tr><td colspan="7" class="empty">Nenhuma solicitação.</td></tr>'}catch(e){console.error(e)}}document.getElementById('solicitacoesBody')?.addEventListener('click',async e=>{const a=e.target.closest('[data-aprovar]'),r=e.target.closest('[data-recusar]');if(a){await api('/api/asaas/solicitacoes-promessa/'+a.dataset.aprovar+'/aprovar',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});carregarSolicitacoesPromessa()}if(r){const resposta=prompt('Motivo da recusa:','Solicitação não aprovada.');if(resposta===null)return;await api('/api/asaas/solicitacoes-promessa/'+r.dataset.recusar+'/recusar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({resposta})});carregarSolicitacoesPromessa()}});$('btnAtualizarSolicitacoes')?.addEventListener('click',carregarSolicitacoesPromessa);
Promise.all([carregarConfig(),carregarEmpresas(),carregarSolicitacoesPromessa()]);
})();
