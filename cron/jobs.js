...
module.exports = pool => {
    ...
    cron.schedule("* * * * *", async () => {
        try {
            const agoraBruto = horarioAgora();
            const agora = normalizarHorario(agoraBruto);
            ...
            const empresas = empresasAtivas.filter(emp => {
                const horarioBanco = normalizarHorario(
                    emp.relatorio_horario || "08:00:00"
                );

                return horarioBanco === agora;
            });

            console.log(`🧭 Horário bruto do Node: "${agoraBruto}"`);
            console.log(`🧭 Horário normalizado: "${agora}"`);
            console.log(
                "🧪 Empresas ativas e horários:",
                empresasAtivas.map(emp => ({
                    id: emp.id,
                    horarioBanco: emp.relatorio_horario,
                    horarioNormalizado: normalizarHorario(
                        emp.relatorio_horario || "08:00:00"
                    )
                }))
            );
